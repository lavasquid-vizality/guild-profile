...
export default function () {
  ...
  if (GFAdd.length) logger.warn({ badge: `vizality://plugins/${this.addonId}/assets/icon.png`, labels: [ ...this._labels, { text: 'Add', color: Constants.Colors.STATUS_GREEN_600 } ], message: GFAdd.join(', ') });
  if (GFRemove.length) logger.warn({ badge: `vizality://plugins/${this.addonId}/assets/icon.png`, labels: [ ...this._labels, { text: 'Remove', color: Constants.Colors.STATUS_RED } ], message: GFRemove.join(', ') });
  if (GFOrder.length && !(GFAdd.length || GFRemove.length)) logger.warn({ badge: `vizality://plugins/${this.addonId}/assets/icon.png`, labels: [ ...this._labels, { text: 'Order', color: Constants.Colors.STATUS_RED } ], message: GFOrder.join(', ') });
}
